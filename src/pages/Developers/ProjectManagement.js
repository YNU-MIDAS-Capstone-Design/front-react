...
function ProjectManagement() {
    ...
    useEffect(() => {
        ...
        try {
            const res = await axios.get("/api/project?page=0&limit=20", {
            ...
            ? res.data.projects.map((p) => ({
                id: p.project_id,
                title: p.title,
                // author: p.author || p.nickname || "작성자 없음",
                }))
            : [];
